              ...
              <div id="resource-img-wrap">
                <div id="resource-img">
                  ...
                  <div id="resource-img-container">
                    <img class="resource-image" :src="originalImage.url" alt />
                    <!-- <a-empty /> -->
                  </div>
                </div>
              </div>
              <div id="identification-result-wrap">
                <div id="identification-result">
                  ...
                  <div id="identification-result-img-container">
                    <div
                      v-for="item in identificationResult.debug[identificationResult.debug.length-1].filePath"
                      :key="item.id"
                    >
                      <img class="identification-result-image" :src="item" alt />
                    </div>
                    <!-- <a-empty /> -->
                  </div>
                  <div class="identification-result-box">
                    <a-icon type="check-circle" :style="{ fontSize: '20px', color: '#08c' }" />
                    <span class="identification-result-message">
                      车牌：
                      <span v-for="item in identificationResult.debug[identificationResult.debug.length-1].recoPlate"
                    ...
                    <span class="identification-result-message">
                      颜色：
                      <span v-for="item in identificationResult.debug[identificationResult.debug.length-1].plateColor"
                      :key="item.id">{{item}}</span>
                    </span>
                  </div>
                </div>
...
export default {
  data() {
    return {
      ...
      originalImage: {
        url:
          "https://ss2.bdstatic.com/70cFvnSh_Q1YnxGkpoWK1HF6hhy/it/u=1014296722,1201237690&fm=26&gp=0.jpg",
      },
      identificationResult: {
        id: 0,
        fileName: "",
  ...
  methods: {
    ...
    processFileTreeData(FileTreeData) {
      ...
      newdebug.push({
        ...
        recoPlate: [FileTreeData.obj.debug[0].recoPlate],
        plateColor: [FileTreeData.obj.debug[0].plateColor],
      });
      ...
      for (let i = 1; i < FileTreeData.obj.debug.length; i++) {
        ...
        if (flag == true) {
          ...
          newdebug[j].recoPlate.push(FileTreeData.obj.debug[i].recoPlate);
          newdebug[j].plateColor.push(FileTreeData.obj.debug[i].plateColor);
        } else {
          newdebug.push({
      ...
      this.originalImage.url = FileTreeData.obj.filePath;

      console.log("a");
      console.log(this.originalImage.url);
      console.log("b");
    },
      ...
      if (data.isLeaf == true) {
        axios({
          ...
          params: {
            ...
            // filePath: "http://124.70.158.103:8080/images/01_蓝牌/2.jpg",
...
.identification-result-box .identification-result-message span {
  margin: 5px;
}
